...
def render_visual_carousel(section_names: List[str], current_index: int, key_prefix: str, preview_data: Dict[str, Any] | None = None) -> int:
    ...
    Returns:
        Selected index after navigation
    """
    # Clean navigation matching ResearchBridge style
    ...
    with col2:
        # Show current section name - ResearchBridge style
        st.markdown(
            f"<div style='text-align: center; padding: 20px 10px; font-size: 1.125rem; font-weight: 500; color: #1a1a1a;'>"
            f"{section_names[current_index]}"
            f"</div>",
            unsafe_allow_html=True
        )
        # Pagination dots - clean styling with blue accent
        dots_html = "<div style='text-align: center; margin-top: 8px;'>"
        for i in range(len(section_names)):
            if i == current_index:
                dots_html += "<span style='color: #1976d2; font-size: 14px; margin: 0 6px;'>●</span>"
            else:
                dots_html += "<span style='color: #bdbdbd; font-size: 14px; margin: 0 6px;'>○</span>"
        dots_html += "</div>"
        st.markdown(dots_html, unsafe_allow_html=True)
